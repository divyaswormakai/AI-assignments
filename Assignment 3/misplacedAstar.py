...
def AstarAlgorithm():
	...
	manhattan.append(CalcMisplacedTiles(start))
		...
		if currState == goal:
			...
			for i in range(1,5):
				...
				if valChange:
					...
					depthList.append(depthList[StateIndex]+1)
					manhattan.append(CalcMisplacedTiles(tempState))
					visited.append(False)
					prevAction.append(i)
					if(tempState==goal):
						break
...
if __name__ == '__main__':
	...
	for i in range(len(totalStates)):
		...
		for j in range(3):
			...
			labelstr+="\n"
		labelstr+="Misplaced: "+str(manhattan[i])+"\n"+"Depth: "+str(depthList[i])
		...
		if i!=0:
			...
			G.add_edge(e)
		G.write_png("E:\\7th Sem\\Artificial Intelligence\\Assignments\\Assignment 3\\misplaced.png")
